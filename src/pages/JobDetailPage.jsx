    ...
    <div className="animate-fadeUp">
      ...
      <div className="card" style={{ padding:'24px 28px', marginBottom:24 }}>
        <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between' }}>
          <div>
            <div style={{ fontFamily:'Syne,sans-serif', fontSize:22, fontWeight:700, color:'var(--navy)' }}>{job?.title}</div>
          ...
          <div style={{ display:'flex', gap:10, alignItems:'center', flexWrap:'wrap', justifyContent:'flex-end' }}>
            {job?.status === 'draft'  && <button className="btn btn-gold btn-sm" onClick={() => handleStatusChange('active')}>Activate</button>}
            {job?.status === 'active' && <button className="btn btn-ghost btn-sm" onClick={() => handleStatusChange('paused')}>Pause</button>}
            {job?.status === 'paused' && <button className="btn btn-gold btn-sm" onClick={() => handleStatusChange('active')}>Resume</button>}
            <button className="btn btn-primary btn-sm" onClick={() => { setShowInvite(true); setInviteLink(''); }}>+ Send Invite</button>
          </div>
        </div>
        ...
        <div style={{ display:'flex', gap:24, marginTop:20, paddingTop:20, borderTop:'1px solid var(--border)' }}>
          {[
            ...
            { label:'Avg Score', val: sessions.filter(s=>s.overall_score).length
                ? (sessions.reduce((a,b) => a+(b.overall_score||0),0) / sessions.filter(s=>s.overall_score).length).toFixed(1)+'/10'
                : '—' },
          ].map(s => (
            <div key={s.label}>
          ...
          <div style={{ display:'flex', gap:8, marginBottom:20, flexWrap:'wrap' }}>
            {CATS.map(c => (
              <button key={c.id} onClick={() => setActiveQ(c.id)} style={{
                ...
                {c.icon} {c.label}
                <span style={{ background: activeQ===c.id?'rgba(255,255,255,0.2)':'var(--border)', color: activeQ===c.id?'#fff':'var(--muted)', padding:'1px 7px', borderRadius:10, fontSize:11, fontFamily:'DM Mono,monospace' }}>
                  {(questions[c.id]||[]).length}
                </span>
              </button>
            ))}
            <button className="btn btn-gold btn-sm" style={{ marginLeft:'auto' }}
              onClick={() => handleGenerate(activeQ)} disabled={!!generating}>
              {generating === activeQ ? <><span className="spinner" /> Generating...</> : '✦ Generate with AI'}
            </button>
          </div>
              ...
              {currentQs.map((q, i) => (
                <div key={q.id} className="card" style={{ padding:'14px 18px' }}>
                  <div style={{ display:'flex', gap:12, alignItems:'flex-start' }}>
                    <span style={{ fontFamily:'DM Mono,monospace', fontSize:11, color:'var(--muted)', background:'var(--surface2)', border:'1px solid var(--border)', padding:'2px 7px', borderRadius:4, flexShrink:0, marginTop:2 }}>Q{i+1}</span>
                    {editId === q.id ? (
        ...
        <div>
          {sessions.length === 0 ? (
            <div className="card"><div className="empty-state">
              ...
              <div className="empty-sub">Send an invite to start.</div>
            </div></div>
          ) : (
            <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
              {sessions.map(s => (
                <Link to={`/sessions/${s.id}`} key={s.id} className="card" style={{ padding:'14px 20px', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
                  <div>
                    <div style={{ fontWeight:600 }}>{s.candidates?.full_name}</div>
                    <div style={{ fontSize:12, color:'var(--muted)', marginTop:3 }}>
                      {s.candidates?.email}{s.completed_at && ` · ${new Date(s.completed_at).toLocaleDateString()}`}
                    </div>
                  </div>
                  <div style={{ display:'flex', gap:10, alignItems:'center' }}>
                    {s.overall_score && <span style={{ fontFamily:'DM Mono,monospace', fontSize:13, fontWeight:600, color:'var(--navy)' }}>{s.overall_score}/10</span>}
                    {s.recommendation && <span className={`badge ${RECO_BADGE[s.recommendation]}`}>{s.recommendation.replace('_',' ')}</span>}
      ...
      {tab === 'invites' && (
        <div>
          <div style={{ display:'flex', justifyContent:'flex-end', marginBottom:16 }}>
            <button className="btn btn-primary" onClick={() => { setShowInvite(true); setInviteLink(''); }}>+ Send Invite</button>
          </div>
          {invites.length === 0 ? (
            <div className="card"><div className="empty-state">
              <div className="empty-icon">◎</div>
              <div className="empty-title">No invites sent yet</div>
            </div></div>
          ) : (
            <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
              {invites.map(inv => (
      ...
      {showInvite && (
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.5)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:1000 }}
          onClick={e => e.target === e.currentTarget && !inviteLink && setShowInvite(false)}>
          <div className="card animate-fadeUp" style={{ width:480, padding:36, maxHeight:'90vh', overflowY:'auto' }}>
            <div style={{ fontFamily:'Syne,sans-serif', fontSize:20, fontWeight:700, marginBottom:6 }}>Send Interview Invite</div>
            <p style={{ color:'var(--muted)', fontSize:13, marginBottom:24 }}>
              A secure interview link will be generated. Share it with the candidate via WhatsApp, Gmail, or any channel.
            </p>
      ...
      {pauseConfirm && (
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.5)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:1000 }}>
          <div className="card animate-fadeUp" style={{ width:480, padding:36 }}>
            <div style={{ fontFamily:'Syne,sans-serif', fontSize:20, fontWeight:700, marginBottom:8, color:'var(--navy)' }}>⚠️ Pause Recruitment?</div>
            <p style={{ color:'var(--muted)', fontSize:14, lineHeight:1.6, marginBottom:20 }}>
              There are <strong style={{ color:'var(--red)' }}>{pauseConfirm.count} candidate{pauseConfirm.count!==1?'s':''}</strong> with pending interview invites.
            </p>
            {pauseConfirm.candidates?.length > 0 && (
              <div style={{ background:'var(--surface2)', border:'1px solid var(--border)', borderRadius:8, padding:14, marginBottom:20, maxHeight:140, overflowY:'auto' }}>
                {pauseConfirm.candidates.map((c, i) => (
                  <div key={i} style={{ fontSize:13, padding:'3px 0', color:'var(--text2)' }}>• {c?.full_name} — {c?.email}</div>
                ))}
              </div>
            )}
            <div style={{ background:'var(--amber-bg)', border:'1px solid #FDE68A', borderRadius:8, padding:'12px 16px', marginBottom:24, fontSize:13, color:'var(--amber-text)' }}>
              All pending invite links will be deactivated and candidates will be notified.
            </div>
            <div style={{ display:'flex', gap:10, justifyContent:'flex-end' }}>
